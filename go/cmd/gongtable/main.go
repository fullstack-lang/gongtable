...
func main() {
	...
	if *marshallOnCommit != "" {
		...
		manualStage = gongtable_fullstack.NewStackInstance(r, "gongtable")
	} else {
		// persistence in a SQLite file on disk
		manualStage = gongtable_fullstack.NewStackInstance(r, "gongtable", "./test.db")
		log.Println("toto")
	}

	gongtable_data.Load(r, gongtable_go.GoModelsDir, "gongtable")

	// generatedStage is the stage generated by filling data with code
	// to test performances
	generatedStage := gongtable_fullstack.NewStackInstance(r, "generated")
