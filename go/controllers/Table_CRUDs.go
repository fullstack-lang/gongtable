...
func (controller *Controller) GetTables(c *gin.Context) {
	...
	for idx := range tableDBs {
		...
		tableDB.CopyBasicFieldsToTable(&tableAPI.Table)
		tableAPI.TablePointersEnconding = tableDB.TablePointersEnconding
...
func (controller *Controller) PostTable(c *gin.Context) {
	...
	// Create table
	tableDB := orm.TableDB{}
	tableDB.TablePointersEnconding = input.TablePointersEnconding
	tableDB.CopyBasicFieldsFromTable(&input.Table)

	query := db.Create(&tableDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) GetTable(c *gin.Context) {
	...
	if err := db.First(&tableDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateTable(c *gin.Context) {
	...
	// fetch the table
	query := db.First(&tableDB, c.Param("id"))
	...
	query = db.Model(&tableDB).Updates(tableDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
